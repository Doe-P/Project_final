...
const { getEducationLevel, getEducationlevelById, insertEducationLevel, updateEducationLevelById, deleteEducationLevelById, getItemsEduc } = require('../models/educationlevelModel');
    ...
    getEducationlevelById(id, (err, result) => {
        if (err) {
            if (err.kind === 'not found') {
                return res.status(404).send({ msg: `Not found educationlevel with id ${id}` });
            } else {
                return res.status(500).send({ msg: " Error retrieving educationlevel with id " + id });
            }
        }
        res.json(result);
    });
...
router.post('/api/v1/education-levels', (req, res) => {
    ...
    if (!req.body.level_id || !req.body.level_name) {
        ...
        insertEducationLevel(data, (err, result) => {
            if (err) {
                return res.status(500).send({ msg: 'Some error occurred while creating the educationlevel' })
            }
            res.json(result);
        });
    }
    ...
    updateEducationLevelById(data, id, (err, result) => {
        if (err) {
            if (err.kind === 'not found') {
                ...
                return res.status(500).send({ msg: 'Error updating educationlevel with id' + id });
            }
        }
        res.json(result)
    });
    ...
    deleteEducationLevelById(id, (err, result) => {
        if (err) {
            if (err.kind === 'not found') {
                ...
                return res.status(500).send({ msg: 'Could not delete educationlevel with id' + id });
            }
        }
        res.json(result);
    });
...
router.get('/api/v1/getItem/education-levels', (req, res) => {
    getItemsEduc((err, result) => {
        if (err) {
            return res.status(500).send({msg: 'Some error occurred while retrieving item educationlevel'})
        }
        res.json(result);
    });
});
