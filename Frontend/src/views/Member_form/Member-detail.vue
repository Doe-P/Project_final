                ...
                <v-row class="mx-10" justify="center">
                  ...
                  <v-col cols="9" class="my-3">
                    ...
                    <v-radio-group v-model="select_riligion">
                      ...
                      <v-radio disabled label="ໝຸດສະລິມ" value="ໝຸດສະລິມ"></v-radio>
                    </v-radio-group>
                  </v-col>
                </v-row>
                <v-row class="mx-10">
...
export default {
  ...
  methods: {
    ...
    async getMember_byID() {
      ...
      try {
        await axios
          .get(`${this.$store.getters.myHostname}/api/v1/members/${this.$route.params.member_detail}`)
          .then((response) => {
    ...
    getTypeMember_item(membertypeid) {
      for (let t = 0; t <= this.typeMember_item_All.length; t++) {
        if (
          ...
          this.selected_typeMember_Item = this.typeMember_item_All[t].typemember;
