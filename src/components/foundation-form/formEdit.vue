  ...
  <div>
    <template>
      <v-row justify="center">
        <v-dialog
          v-model="this.$store.getters.getfound_formEdit"
          persistent
          max-width="500px"
        ...
        >
          <v-card>
            ...
            <v-card-text>
              <v-container>
                <v-form class="table-content" v-model="valid" @submit.prevent="submit_edit">
                  <v-text-field
                    label="ຮາກຖານ"
                    required
                     :rules="[required('ຊື່ຮາກຖານ'), minLength('ຊື່ຮາກຖານ',5), maxLength('ຊື່ຮາກຖານ',30)]"
                  ...
                  <v-btn @click="close_form" color="error"
                    >ຍົກເລີກ</v-btn
                  >
                  <span></span>
                  <v-btn 
  ...
  data() {
    return {
      ...
      minLength(propertyType,minlength){
        return v => v && v.length >= minlength || `${propertyType}ຕ້ອງຫຼາຍກວ່າຫຼືເທົ່າກັບ ${minlength}`
      },
      maxLength(propertyType,maxlength){
        return v => v && v.length <= maxlength || `${propertyType}ຕ້ອງໜ້ອຍກວ່າຫຼືເທົ່າກັບ ${maxlength}`
      },
      // valid form
      valid:false,
